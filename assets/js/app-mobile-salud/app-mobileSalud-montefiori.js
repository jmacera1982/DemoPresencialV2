// Toggle DNI visibility
    ...
    /* Cambio de plan + videollamada */
    (function () {
      function activarAsesoramiento() {
        if (typeof window.abrirModalAtencionMF === 'function') {
          window.abrirModalAtencionMF();
        }
      }

      var btnCerrarPromoPlan = document.getElementById('btnCerrarPromoPlan');
      if (btnCerrarPromoPlan) {
        btnCerrarPromoPlan.addEventListener('click', function (e) {
          e.stopPropagation();
          var p = document.getElementById('mfPromoPlan');
          if (p) p.style.display = 'none';
        });
      }

      var promoPopup = document.getElementById('mfPromoPopup');
      var PROMO_DISMISS_KEY = 'mfPromoPlanDismissed';
      ...
      var promoBackdrop = document.getElementById('mfPromoPopupBackdrop');
      if (promoBackdrop) promoBackdrop.addEventListener('click', function () { cerrarPromoPopup(true); });

      var mfPromoPlan = document.getElementById('mfPromoPlan');
      if (mfPromoPlan) {
        mfPromoPlan.addEventListener('click', function (e) {
          if (e.target.closest('#btnCerrarPromoPlan')) return;
          irAVideollamadaDesdePromo();
        });
        mfPromoPlan.addEventListener('keydown', function (e) {
      ...
      if (!dismissed) {
        setTimeout(abrirPromoPopup, 1200);
      }
    })();
    ...
    (function () {
      ...
      window.mfCurrentVideoCallUrl = 'about:blank';

      var modalEl = document.getElementById('mfModalAtencion');
      var modalBackdrop = document.getElementById('mfModalAtencionBackdrop');
      var modalClose = document.getElementById('mfModalAtencionClose');
      var fvFormAlert = document.getElementById('mfFvFormAlert');
      var fvTurnInfoSection = document.getElementById('mfFvTurnInfoSection');
      var fvLoadingEl = document.getElementById('mfFvLoading');
      ...
      function fvResetModal() {
        ...
        var vf = document.getElementById('mfVideoFrame');
        if (vf) vf.src = 'about:blank';
        window.mfCurrentVideoCallUrl = 'about:blank';
        var vm = document.getElementById('mfVideoModal');
        if (vm) {
      ...
      async function fvEnviarAtencion() {
        ...
        try {
          ...
          window.mfCurrentVideoCallUrl = (out.jsonDetails && out.jsonDetails.videoCallUrl) || 'about:blank';

          if (fvLoadingEl) fvLoadingEl.style.display = 'none';
          fv$('mfFvTurnCodeDisplay').textContent = turnCode;
          fv$('mfFvTurnNumber').textContent = turnNumber;
          fv$('mfFvAvgWaitingTime').textContent = isFinite(avgWait) ? Math.floor(parseFloat(avgWait)) + ' min' : 'N/A';
          fv$('mfFvQueueName').textContent = queueName;
          fv$('mfFvWaitingRoomName').textContent = wrName;
          fv$('mfFvTurnStatus').textContent = 'Consultando...';
          fv$('mfFvTurnStatus').className = 'mf-fv-info-value mf-fv-status-warn';
          fv$('mfFvCurrentWaitingTime').textContent = 'N/A';
          fv$('mfFvBtnAbrirVideo').disabled = true;
      ...
      window.abrirModalAtencionMF = function () {
        if (!modalEl) return;
        fvResetModal();
      ...
      function fvStartPolling(turnCode) {
        ...
        var mapStatus = function (s) {
          if (s === 'ANNOUNCED') return { text: 'Lo estamos llamando', cls: 'mf-fv-status-ok' };
          if (s === 'WAITING_TO_BE_CALLED') return { text: 'En breve lo llamaremos', cls: 'mf-fv-status-warn' };
          if (s === 'FINALIZED') return { text: 'Atención finalizada', cls: 'mf-fv-status-done' };
          return { text: s || 'Desconocido', cls: 'mf-fv-status-warn' };
        };
        ...
        var tick = async function () {
          try {
            ...
            fv$('mfFvTurnStatus').textContent = m.text;
            fv$('mfFvTurnStatus').className = 'mf-fv-info-value ' + m.cls;
            var avg = (data.averageWaitingTime != null ? data.averageWaitingTime : data.serviceTime);
            if (avg !== undefined && avg !== null && !isNaN(avg)) {
              fv$('mfFvCurrentWaitingTime').textContent = Math.floor(parseFloat(avg)) + ' min';
            }
            if (data.status === 'ANNOUNCED') {
              fv$('mfFvBtnAbrirVideo').disabled = false;
            }
          } catch (e) {
            fv$('mfFvTurnStatus').textContent = 'Error al consultar';
            fv$('mfFvTurnStatus').className = 'mf-fv-info-value mf-fv-status-err';
          }
        };
      ...
      var fvBtnAbrirVideo = document.getElementById('mfFvBtnAbrirVideo');
      var mfVideoModal = document.getElementById('mfVideoModal');
      if (fvBtnAbrirVideo) {
        fvBtnAbrirVideo.addEventListener('click', function () {
          var videoUrl = window.mfCurrentVideoCallUrl || 'about:blank';
          var urlWithParam = videoUrl.indexOf('?') >= 0 ? videoUrl + '&videocallUser=mobile' : videoUrl + '?videocallUser=mobile';
          var vf = document.getElementById('mfVideoFrame');
          if (vf) vf.src = urlWithParam;
          if (mfVideoModal) {
            mfVideoModal.classList.add('active');
            mfVideoModal.setAttribute('aria-hidden', 'false');
          }
        });
      }

      var mfBtnCloseVideo = document.getElementById('mfBtnCloseVideo');
      if (mfBtnCloseVideo && mfVideoModal) {
        mfBtnCloseVideo.addEventListener('click', function () {
          mfVideoModal.classList.remove('active');
          mfVideoModal.setAttribute('aria-hidden', 'true');
        });
        mfVideoModal.addEventListener('click', function (e) {
          if (e.target === mfVideoModal) {
            mfVideoModal.classList.remove('active');
            mfVideoModal.setAttribute('aria-hidden', 'true');
          }
        });
      ...
      document.addEventListener('keydown', function (e) {
        ...
        if (mfVideoModal && mfVideoModal.classList.contains('active')) {
          mfVideoModal.classList.remove('active');
          mfVideoModal.setAttribute('aria-hidden', 'true');
